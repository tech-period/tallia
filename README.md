...
オブジェクトマスタ画面から、そのプロジェクトのオブジェクトをカテゴリ・タグ・イメージ画像ごと
`.tallia` ファイルに書き出し、別のプロジェクトへ取り込めます。バックアップとは別の形式です。

|        | バックアップ                 | 移し替え                           |
| ------ | ---------------------------- | ---------------------------------- |
| 目的   | 同じデータをそのまま復元する | 別のプロジェクトへ定義を配る       |
| 対象   | 全ストア                     | オブジェクト・カテゴリ・タグ・画像 |
| ID     | ファイルに含む               | 含まない（名前で突き合わせる）     |
| 拡張子 | `.json`                      | `.tallia`                          |

取り込み先は「今表示しているプロジェクト」で、突合キーはオブジェクト名です。
同じ名前のオブジェクトがあったときの扱いは、そのままにする / ファイルの内容で上書きする
の 2 つから選べます。**どちらでも既存のオブジェクトが削除されることはありません。**
カテゴリ・タグは名前で解決し、取り込み先に無ければ新しく作ります。
